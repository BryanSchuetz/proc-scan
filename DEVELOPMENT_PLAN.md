...
Status: merged dgMarket/MCCDGMarket Browser Run adapter deployed and verified
...
- The inclusive baseline, DAI-Fit/Miss-Fit evidence weights, and final Addressable fit-score threshold are shared across every Source and Client. Each Source integration supplies the applicable Client Minimum Value Floors and any justified structured hard exclusions; value never contributes score.
...
- Addressability applies the Source- and Client-specific Minimum Value Floor as a hard gate, then scores shared DAI-Fit and Miss-Fit language evidence.
- Addressability uses explicit, project-owned, configurable rules rather than an AI model's final judgment.
- The configuration defines hard exclusions, weighted DAI-Fit/Miss-Fit criteria, and the final fit-score threshold.
...
- Automated scan cycles run at 6:00 AM and 6:00 PM in `Europe/London`, following daylight-saving changes.
...
Cloudflare schedules use UTC. To preserve 6:00 AM and 6:00 PM `Europe/London` across daylight-saving changes, trigger at the possible UTC equivalents, gate execution on the local time, and enforce one unique scan-run key per local date and AM/PM cycle.
...
The implementation workflow, normalization fields, shared scoring boundary, verification requirements, and parallel-orb ownership model are documented in [`SOURCE_INTEGRATION_GUIDE.md`](SOURCE_INTEGRATION_GUIDE.md).
...
- Add a separate addressability configuration containing a schema version, hard exclusions, Source/Client Minimum Value Floors, weighted fit criteria, threshold, and stable rule IDs. Store matched rule IDs and score contributions on each retained event.
...
### Phase 1 — Unauthenticated public access
...
| 2 | TED | [Current search](https://ted.europa.eu/en/search/result?query=%28funding+IN+%28external-aid-program%29%29+AND+%28buyer-name+IN+%28AGRI+CLIMA+ECHO+CINEA+GROW+IDEA+REA+INTPA+DEV+ENEST+MENA+TRADE%29%29+SORT+BY+publication-number+DESC&scope=ACTIVE&onlyLatestVersions=false&sortColumn=publication-number&sortOrder=DESC&page=1) | Official Search API adapter is implemented with iteration pagination, active external-aid and 12-DG Client scoping, planning/competition mapping, explicit changes, procedure values, and a €1 million EUR floor. If no tender or participation deadline exists, BT-127's estimated contract-notice publication date is retained as a provenance-labelled proxy due date. XML/eForms enrichment remains optional follow-up work. |
| 3 | EU Funding & Tenders Portal | [Portal](https://ec.europa.eu/info/funding-tenders/opportunities/portal/screen/home) | Official Search API adapter is deployed for forthcoming/open calls for tenders, with complete-snapshot pagination, the same 12-Client scope and €1 million EUR floor as TED, structured authority filtering, and formal corrigenda. Grant topics remain omitted because no authoritative responsible DG is exposed. |
| 4 | dgMarket | [Advanced search](https://www2.dgmarket.com/advanced-search) | Browser-backed adapter is enabled for MCC and MCA notices from the general dgMarket search and [MCCDGMarket](https://mcc.dgmarket.com/). It merges exact notice-ID overlaps before applying the $500,000 USD floor, preserving the general portal's structured fields and the MCC portal's public descriptions. Permission for systematic access has been confirmed. Migration `0021` moves the adapter to Browser Run and adds the MCC channel; a production scan successfully processed all 21 current MCC-channel candidates. |
| 5 | FMO Open Tenders | [Open Tenders](https://www.fmo.nl/open-tenders) | Direct-HTML full-snapshot adapter is enabled for FMO as Client, with reference-number identity, CET/CEST date normalization, maximum-budget extraction, normal shared fit scoring, and a €500,000 EUR floor. Migration `0008` enables the Source following owner authorization on August 31, 2026. |
| 6 | European Investment Bank | [All procurement](https://www.eib.org/en/about/procurement/all/index?sortColumn=configuration.contentStart&sortDir=desc&pageNumber=0&itemPerPage=25&pageable=true&la=EN&deLa=EN&orYearTo=true&orYearFrom=true&procurementStatus=All&_g_procurementInformations_type=call-technical-assistance&or_g_procurementInformations_type=true) | First-party-JSON adapter is enabled for active TA Operations, with EIB detail/history retrieval, batched official TED enrichment, normal shared fit scoring, and a €500,000 EUR floor. Migration `0010` enables the Source following confirmation of permission for programmatic access on August 31, 2026. |
| 7 | DEFRA Atamis | [Public opportunities](https://atamis-9529.my.salesforce-sites.com/?searchtype=Projects) | Public-CSV full-snapshot adapter is enabled. It retains non-closed records with current response dates, maps the DEFRA-family catalogue to Client DEFRA, applies normal shared fit scoring, and configures a £250,000 GBP floor. The public feed exposes neither values nor individual contracting authorities, so those fields remain missing pending authoritative enrichment. |
| 8 | DEZNZ Jaggaer | [Current opportunities](https://beisgroup.ukp.app.jaggaer.com/esop/guest/go/public/opportunity/current) | Direct-HTML full-snapshot adapter is deployed and enabled after the owner confirmed permission for programmatic access on September 3, 2026. It uses a fresh ephemeral public visitor session, retains Services only, disambiguates DESNZ from the shared DSIT buyer using detail text, normalizes exact and lot-level GBP estimates, and configures a £250,000 GBP floor. Supplier credentials are not required for discovery; the first production scan is pending. |
| 9 | ECEPP | [Search opportunities](https://ecepp.ebrd.com/delta/noticeSearchResults.html) | Deployed public-HTML full-snapshot adapter requires future closing dates, retains EBRD consultancy/advisory notices, and applies separate €/$250,000 floors to known Opportunity values without treating project-level estimates as Opportunity values. Its first successful production scan retained eight future-dated events. |
| 10 | FCDO Jaggaer Public | [Current opportunities](https://fcdo.bravosolution.co.uk/esop/guest/go/public/opportunity/current) | Deployed direct-HTML full-snapshot adapter uses a fresh ephemeral visitor session, retains FCDO Services competitions and explicitly labelled early-market-engagement activities, excludes frameworks and call-downs, and applies a £250,000 floor to known GBP values. The public request flow has completed successfully in production; early-market-engagement inclusion awaits its next scan. |
| 11 | SIMAP | [Search](https://www.simap.ch/en) | Official public-API adapter is enabled. It searches SDC and SECO service tenders with rolling pagination, requires matching procurement-office names, maps multilingual details and corrected tenders, and applies a CHF 1 million floor to known CHF values. Login and two-factor authentication are not required for publication search or detail access. |

### Phase 2 — Unattended authentication

| Source | Starting URL | Access | Plan |
|---|---|---|---|
| SAM.gov | [SAM.gov](https://sam.gov/) | API key | Deployed as the first Source; resolve the current quota constraint, validate scheduled live scans, and decide bulk versus selective description enrichment. |
...
| Source | Starting URL | Access | Plan |
|---|---|---|---|
| FCDO Jaggaer Frameworks | [Login](https://fcdo.bravosolution.co.uk/web/login.html) | Login + Google Authenticator | Build a separate Source for invitation-only technical-assistance framework opportunities with no value floor. Reuse an approved manually established session while valid; stop and request reauthentication on expiry. |
...
- Run SAM.gov, Grants.gov, TED, and EU Funding & Tenders on the production schedule behind Cloudflare Access.
...
- Implement FCDO Frameworks with explicit `reauthentication_required` state and runbook.
