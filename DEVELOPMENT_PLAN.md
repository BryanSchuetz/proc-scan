...
Status: DEZNZ Jaggaer ready to deploy enabled; DEFRA Atamis, EIB, and FMO enabled; dgMarket paused
...
## Implementation Status — September 3, 2026

The local foundation includes the React/Worker application, D1 migrations and FTS5 index, fixture seed, read-only paginated API and table, deterministic classifier/config schemas, event identity and inheritance logic, OCDS-shaped release mapping, Cloudflare Access JWT guard, and New York-time Workflow schedule gate. SAM.gov, Grants.gov, TED, EU Funding & Tenders, dgMarket, FMO, EIB, DEFRA Atamis, and DEZNZ Jaggaer have registered adapters; dgMarket is paused, DEZNZ Jaggaer is staged for enabled deployment, and the other seven Sources are enabled in production. SAM.gov's official v2 API adapter implements bounded cursor overlap, organization-scoped page-index pagination, credential-safe failures, and notice normalization. Grants.gov's official unauthenticated adapter discovers its current agency hierarchy, crosswalks it to the same approved federal organizations, scans only forecasted and posted opportunities, paginates a complete active snapshot, and enriches approved results through `fetchOpportunity` for synopsis/forecast text, eligibility, and funding values. TED's official unauthenticated v3 Search API adapter scans a consistent active external-aid snapshot for 12 approved Directorate-General Clients, retains planning and competition forms, maps explicit change notices, and normalizes multilingual descriptions, deadlines, and procedure estimates. The EU Funding & Tenders adapter uses the Commission's official anonymous Search API to scan forthcoming and open tender calls, filters structured lead contracting authorities to the same 12 Clients as TED, maps formal corrigenda, and normalizes dates, CPV metadata, and procedure estimates. Grant topics are omitted because the Search API does not expose an authoritative responsible Directorate-General for the required Client filter. The dgMarket adapter parses its public server-rendered search, maintains session pagination, selects MCA and EU member-state government buyers in the Consultancy category, and uses a cursor with bounded date overlap. The FMO adapter parses one complete server-rendered Open Tenders page snapshot and normalizes reference numbers, descriptions, dates, and maximum budgets. The EIB adapter combines EIB's first-party procurement JSON with one batched official TED search to normalize active TA Operations, descriptions, exact deadlines, and estimated values. The DEFRA Atamis adapter reads the complete public catalogue CSV, removes closed and stale undated listings, and normalizes stable references, descriptions, categories, and dates without requiring supplier login. The DEZNZ Jaggaer adapter reads a complete public listing and sequential public detail pages, retains Services, disambiguates DESNZ from the shared DSIT buyer, and normalizes labelled GBP values and UK-local deadlines without using supplier credentials. All nine adapters use shared classification, persistence, deduplication, and per-Source run accounting.
...
| 4 | dgMarket | [Advanced search](https://www2.dgmarket.com/advanced-search) | Direct-HTML adapter is deployed for MCC-funded MCA notices and Consultancy notices from government buyers whose notice-contact country is one of the 27 EU member states. It uses session pagination, a 90-day initial window, two-day cursor overlap, and separate $500,000 USD and €1 million EUR floors. Permission for systematic access has been confirmed. Migration `0007` pauses the Source for result-set noise review; its records and cursor remain stored but hidden. |
...
| 8 | DEZNZ Jaggaer | [Current opportunities](https://beisgroup.ukp.app.jaggaer.com/esop/guest/go/public/opportunity/current) | Direct-HTML full-snapshot adapter is ready to deploy enabled after the owner confirmed permission for programmatic access on September 3, 2026. It uses a fresh ephemeral public visitor session, retains Services only, disambiguates DESNZ from the shared DSIT buyer using detail text, normalizes exact and lot-level GBP estimates, and configures a £250,000 GBP floor. Supplier credentials are not required for discovery. |
