...
Status: FMO adapter implemented but disabled pending access approval; dgMarket remains paused
...
- **Before enabling FMO**: confirmation that the intended internal metadata ingestion and republication is permitted under FMO's website terms or by direct permission.
...
| 5 | FMO Open Tenders | [Open Tenders](https://www.fmo.nl/open-tenders) | Direct-HTML full-snapshot adapter is implemented for FMO as Client, with reference-number identity, CET/CEST date normalization, maximum-budget extraction, normal shared fit scoring, and a €500,000 EUR floor. Migration `0008` registers the Source disabled pending confirmation of access and reuse permission. |
