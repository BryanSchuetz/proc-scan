...
Status: Addressability v6 deployed; production reclassification pending; latest SAM.gov scan quota-limited
...
The Worker, static assets, Workflow, production D1 database, migrations through Grants.gov, SAM.gov secret, and Worker-level Cloudflare Access application are deployed. Access uses the same 12-hour policy as DAI CV Formatter: `@dai.com` users and the configured owner account can authenticate through the account's Cloudflare or email one-time PIN identity providers. The Worker independently validates the Access issuer and application audience. Addressability version 6 is deployed; version 7 and the TED migration are implemented locally but not deployed. Production rows have not yet been rescanned under version 6: Grants.gov currently has 41 Addressable and 4 Uncertain version-5 records, while SAM.gov's 63 retained records remain assessed with version 3 after its request quota was exhausted on the latest cycle. A successful scan after deployment will apply version 7 to rediscovered records. The first scheduled cycle remains to be confirmed, and no email provider has been configured. SAM.gov description and pre-award value enrichment remain unresolved because descriptions consume additional keyed requests and the public search schema has no estimated solicitation value.
