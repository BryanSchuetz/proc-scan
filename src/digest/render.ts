...
function plainText(digest: PreparedDigest, appUrl?: string): string {
  const completed = digest.sourceRuns.filter((source) => source.status === "completed");
  ...
  const lines = [
    ...
    `Last scan: ${formatDate(digest.completedAt, true)}`,
    `Scan coverage: ${completed.length} of ${digest.sourceRuns.length} sources completed`,
    `Completed sources: ${completed.map((source) => source.sourceName).join(", ") || "None"}`,
    ...(failed.length > 0
      ? [`Failed sources: ${failed.map((source) => source.sourceName).join(", ")}`]
      : []),
    "",
  ];
  let currentClient = "";
...
export function renderDigest(digest: PreparedDigest, appUrl?: string): DigestMessage {
  ...
  const coverage = failed.length > 0
    ...
    : "";
  const scanSummary = `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin-top:18px;border-collapse:collapse;background:#D3E3F0;border-left:3px solid #1E7AB3;font:13px/1.5 Arial,sans-serif;color:#404D59">
    <tr>
      <td width="43%" style="padding:12px 14px 10px;vertical-align:top"><span style="font-size:10px;font-weight:700;letter-spacing:.04em;text-transform:uppercase;color:#56687B">Last scan</span><br><strong>${escapeHtml(formatDate(digest.completedAt, true))}</strong></td>
      <td width="57%" style="padding:12px 14px 10px;vertical-align:top"><span style="font-size:10px;font-weight:700;letter-spacing:.04em;text-transform:uppercase;color:#56687B">Scan coverage</span><br><strong>${completed.length} of ${digest.sourceRuns.length} sources completed</strong></td>
    </tr>
    <tr><td colspan="2" style="padding:10px 14px 12px;border-top:1px solid #B5C9D8"><span style="font-size:10px;font-weight:700;letter-spacing:.04em;text-transform:uppercase;color:#56687B">Completed sources</span><br>${escapeHtml(completed.map((source) => source.sourceName).join(", ") || "None")}</td></tr>
  </table>`;
  ...
  <body style="margin:0;padding:0;background:#F3F4F5">
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;background:#F3F4F5"><tr><td align="center" style="padding:24px 12px">
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="width:100%;max-width:680px;border-collapse:collapse">
        ...
        <tr><td style="padding:18px 30px;background:#D3E3F0;font:13px/1.5 Arial,sans-serif;color:#404D59">Generated by the DAI Procurement Registry after the ${escapeHtml(formatDate(digest.completedAt, true))} scan.</td></tr>
      </table>
    </td></tr></table>
  </body></html>`;
